...
class CalibrationWidget(QWidget):
    ...
    def __init__(self, img, pixel1, pixel2, mask=None, cal=None, dist=1.0, rot1=0.0, rot2=0.0, rot3=0.0, wavelength=1.0,
                 splineFile=None, parent=None):
        """
        ...
        self.geo = Geometry(dist=self.dist, poni1=self.poni1, poni2=self.poni2, rot1=self.rot1, rot2=self.rot2,
                            rot3=self.rot3, pixel1=self.pixel1, pixel2=self.pixel2, wavelength=self.wavelength * 1e-10,
    ...
    def ring_color_width_changed(self):
        try:
            for key in self.rings.keys():
                self.rings[key].setPen(
                    pg.mkPen(color=self.calRingColorButton.color(), width=int(self.calRingWidthLineEdit.text())))
        except:
    ...
    def applyFit2D(self):
        ...
        self.geo.setFit2D(self.dist * 1e3, self.beamPosX, self.beamPosY, tilt=self.tilt,
                          tiltPlanRotation=self.tiltPlaneRot)
        ...
        self.directBeam.setData([self.beamPosX], [self.beamPosY], symbol='+',
                                symbolPen=pg.mkPen(color=self.DBColorButton.color()),
                                symbolBrush=pg.mkBrush(color=self.DBColorButton.color()), symbolSize=10)
        self.SAdirectBeam.setData([self.beamPosX], [self.beamPosY], symbol='+',
                                  symbolPen=pg.mkPen(color=self.DBColorButton.color()),
                                  symbolBrush=pg.mkBrush(color=self.DBColorButton.color()), symbolSize=10)
        self.pixel2QMap()
    ...
    def create_rawImageDock(self):
        ...
        self.directBeam = pg.PlotDataItem([self.beamPosX], [self.beamPosY], symbol='+',
                                          symbolPen=pg.mkPen(color=self.DBColorButton.color()),
                                          symbolBrush=pg.mkBrush(color=self.DBColorButton.color()), symbolSize=10)
    ...
    def create_SAImageDock(self):
        ...
        self.SAdirectBeam = pg.PlotDataItem([self.beamPosX], [self.beamPosY], symbol='+',
                                            symbolPen=pg.mkPen(color=self.DBColorButton.color()),
                                            symbolBrush=pg.mkBrush(color=self.DBColorButton.color()), symbolSize=10)
        ...
        if str(self.calibrantComboBox.currentText()) != 'New':
            ...
            for line in lines:
                ...
                if line[0] != '#':
                    ...
                    self.calibrantListWidget.addItem(
                        'Peak %d: d=%.3f Angs, q=%.3f inv Angs ' % (i, self.cal_dvals[i], self.cal_qvals[i]))
    ...
    def save_calibrant(self):
        fname = QFileDialog.getSaveFileName(self, 'Provide a filename for the calibrant',
                                            directory=os.path.join(CWD, 'calibrants'), filter='Calibrant file (*.D)')[0]
        if fname[-2:] != '.D':
            fname = fname + '.D'
    ...
    def create_calibrationDock(self):
        ...
        self.pointTreeWidget.header().setSectionResizeMode(QHeaderView.ResizeToContents)
    ...
    def checkCalibration(self):
        ...
        """
        ai = AzimuthalIntegrator(dist=self.dist, poni1=self.poni1, poni2=self.poni2, rot1=self.rot1, rot2=self.rot2,
                                 rot3=self.rot3, pixel1=self.pixel1, pixel2=self.pixel2,
                                 wavelength=self.wavelength * 1e-10, splineFile=self.splineFile)
        cakedArray, qr, phir = ai.integrate2d(self.imgData, 1000, error_model='poisson', mask=self.maskData, dark=None,
                                              unit='q_A^-1')
        xmin = qr[0]
        xmax = qr[-1]
        ymin = phir[0]
    ...
    def points_picked(self, evt):
        ...
        try:
            ...
            self.pointsPlotItems[val] = pg.PlotDataItem(symbol='o', symbolSize=10, symbolBrush=pg.mkBrush(
                color=self.pointColorWidget[val].color()), pen=None)
        ...
        if evt.button() == Qt.LeftButton:
            ...
            if (0 <= x1 <= Nx) and (0 <= y1 <= Ny):
                ...
                self.pointsPlotItems[val].setData(np.array(self.points[val]), symbol='o', symbolSize=10,
                                                  symbolBrush=pg.mkBrush(color=self.pointColorWidget[val].color()),
                                                  pen=None)
            else:
    ...
    def peak_search(self, x, y, method=None):
        ...
        if method == 'None':
            ...
            chunk = self.imgData.T[np.max([x - self.win, 0]):np.min([x + self.win, Nx - 1]),
                    np.max([y - self.win, 0]):np.min([y + self.win, Ny])]
            ...
            return np.max([x - self.win, 0]) + np.sum(yc * chunk) / np.sum(chunk), np.max([y - self.win, 0]) + np.sum(
                xc * chunk) / np.sum(chunk)

    def updatePoints(self, btn):
        key = [k for k in self.pointColorWidget.keys() if self.pointColorWidget[k] == btn]
        self.pointsPlotItems[key[0]].setData(np.array(self.points[key[0]]), symbol='o', symbolSize=10,
                                             symbolBrush=pg.mkBrush(color=self.pointColorWidget[key[0]].color()),
                                             pen=None)
        ...
        for item in items:
            if str(item.text(0))[:4] != 'Peak':
                ...
                if len(self.points[key]) != 0:
                    self.pointsPlotItems[key].setData(np.array(self.points[key]), symbol='o', symbolSize=10,
                                                      symbolBrush=pg.mkBrush(color=self.pointColorWidget[key].color()),
                                                      pen=None)
                else:
                    self.rawImageWidget.imageView.getView().vb.scene().removeItem(self.pointsPlotItems[key])
                    self.pointsPlotItems[key] = pg.PlotDataItem(symbol='o', symbolSize=10, symbolBrush=pg.mkBrush(
                        color=self.pointColorWidget[key].color()), pen=None)
                ...
                self.rawImageWidget.imageView.getView().vb.scene().removeItem(
                    self.pointsPlotItems[int(str(item.text(0)).split()[1])])
    ...
    def calibrate(self):
        if sum([len(self.points[key]) for key in self.points.keys()]) >= 6:
            ...
            # self.pixel2QMap()
            # self.calibration_peak_selection_changed()
            self.rawImageWidget.imageView.getView().setTitle(
                title='<span><font size=\"' + '3' + '\">' + '&#967;<sup>2</sup>=%.3e, Red-&#967;<sup>2</sup>=%.3e' % (
                out.chisqr, out.redchi) + '</font></span>')
            self.q_points = self.cal_q_points(fparam)
            self.solid_angle_plot()
            QMessageBox.information(self, 'Calibration Info', 'Calibration is done with the provided points',
                                    QMessageBox.Ok)
            # self.solid_angle=self.geo.solidAngleArray(shape=self.imgData.shape)
        else:
            QMessageBox.warning(self, 'Calibration Error', 'Please select 6 or more points to perform the calibration',
                                QMessageBox.Ok)
    ...
    def save_calibration(self):
        fname = str(
            QFileDialog.getSaveFileName(self, 'Calibration file', directory=CWD, filter='Clibration files (*.poni)')[0])
