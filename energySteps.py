...
if len(sys.argv)==6:
    ...
    for i in range(steps):
       # print("%.5f\t%.3f"%(f1vals[i],e1vals[i]/1e3))
        ...
        print("%.5f\t%.3f\t%.3f"%(f1vals[i],e1vals[i]/1e3,e1vals[i]/1e3+0.17),xrdb.f1_chantler(element=element,energy=e1vals[i],smoothing=0))
    fh=open(fname,'w')
    txt='Energy ['+evaltxt[:-1]+'] absolute coupled'
    fh.write(txt)
    fh.close()
    pl.plot(e1vals,xrdb.f1_chantler(element=element,energy=e1vals,smoothing=0),'go')
    pl.show()
else:
