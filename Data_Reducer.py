...
class Data_Reducer(QWidget):
    ...
    def create_UI(self):
        ...
        row+=1
        col=0
        nptLabel=QLabel('Npts')
        self.layout.addWidget(nptLabel,row,col)
        col+=1
        self.nptLineEdit=QLineEdit(str(self.npt))
        self.nptLineEdit.returnPressed.connect(self.nptChanged)
        self.layout.addWidget(self.nptLineEdit,row,col)
        col+=1
    ...
    def nptChanged(self):
        ...
        """
        try:
            self.npt=int(self.nptLineEdit.text())
        except:
...
if __name__=='__main__':
    ...
    w.setGeometry(200,2000,800,800)
