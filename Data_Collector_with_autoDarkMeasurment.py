...
class Data_Collector(QWidget):
    ...
    def create_dataColDock(self):
        ...
        self.dataColLayout.addWidget(self.collectDarkButton,row=row,col=0,colspan=2)
        self.dataColLayout.addWidget(self.staticCollectButton,row=row,col=2,colspan=2)
        self.dataColLayout.addWidget(self.dynamicCollectButton,row=row,col=4,colspan=2)
    ...
    def addDetector(self,detname=None):
        ...
        if detname not in self.usedDetectors:
            self.detectorWidgets[detname]=Detector_Widget()
    ...
    def openExperiment(self,expFolder=None):
        ...
        if expFolder is not None:
            ...
            self.experimentFolder=str(QFileDialog.getExistingDirectory(self,caption='Open existing experiment folder',directory='/home/epics/CARS5/Data/Data/saxs'))
            self.experimentFolderLineEdit.setText(self.experimentFolder)
        self.experimentLogFile=os.path.join(self.experimentFolder,'experiment.log')
            ...
            if str(self.sampleNameLineEdit.text())!='':
                ...
                if os.path.exists(self.sampleFolder):
                    ...
                    if ans==QMessageBox.Yes:
                        ...
                        self.experimentLogHandle.write('##Sample folder added/accessed %s'%self.sampleFolder)
                    else:
    ...
    def dynamic_collect(self):
        ...
        if str(self.dynamicCollectButton.text())!='Abort':
            ...
            if limitsOK:
                ...
                self.measurementProgressDialog.setValue(self.measurementCount)
            else:
    ...
    def pre_count(self):
        ...
        for detname in self.usedDetectors:
            ...
            if self.darkImage:
                caput(self.detectors[detname]['PV']+'FrameType',1) #Setting the Frame type to background
            else:
                caput(self.detectors[detname]['PV']+'FrameType',0) # Setting the Frame type to normal mode
    ...
    def count_em(self):
        ...
        if self.darkImage:
            ...
            self.instrumentStatus.setText('Collectting data from all the Area Detectors. Please wait...')
        self.counting=True
        for detname in self.usedDetectors:
    ...
    def post_count(self):
        ...
        for detname in self.usedDetectors:
            ...
            if not self.darkImage:
                ...
                file.header['Time']=os.path.getctime(os.path.join(self.experimentFolder,'img_0001.tif'))
