...
class Sphere_Uniform_Edep: #Please put the class name same as the function name
    def __init__(self, x=0, Np=10, flux=1e13, dist='Gaussian', relement='Au', Energy=None, NrDep='True', norm=1.0, mpar={'Material':['Au','H2O'],'Density':[19.32,1.0],'Sol_Density':[1.0,1.0],'Rmoles':[1.0,0.0],'R':[1.0,0.0],'Rsig':[0.0,0.0]}):
        """
        Documentation
    ...
    def y(self):
        ...
        if type(self.x) == dict:
            ...
            for key in self.x.keys():
                ...
                sqf[key]=self.norm*np.array(sq)*6.022e20
