...
class UFO_Disc: #Please put the class name same as the function name
    def __init__(self, x=0, Np=10, flux=1e13, term='Total',dist='Gaussian', Energy=None, relement='Au', NrDep='True', norm=1.0, thickness=1.0, sbkg=0.0, cbkg=0.0, abkg=0.0, D=1.0, phi=0.1, U=-1.0, SF='None', mpar={'Material':['Au','H2O'],'Density':[19.32,1.0],'Sol_Density':[1.0,1.0],'Rmoles':[1.0,0.0],'R':[1.0,0.0],'Rsig':[0.0,0.0]}):
        """
        Documentation
    ...
    def y(self):
        ...
        density=np.where(r>self.thickness/2,(np.array(self.__density__)-self.__density__[-1])*np.arcsin(self.thickness/2/(r+np.array(self.__R__)/2))/np.pi
                                                                                                        +self.__density__[-1],np.array(self.__density__))
        ...
        if type(self.x)==dict:
            ...
            for key in self.x.keys():
                sqf[key] = self.norm * 6.022e20 * self.new_sphere_dict(tuple(self.x[key]),tuple(self.__R__),self.__Rsig__[0],
            ...
            key1='Total'
            total = self.norm * 6.022e20*struct *self.new_sphere_dict(tuple(self.x[key]), tuple(self.__R__),self.__Rsig__[0],
            ...
            tsqf,eisqf,asqf,csqf=self.new_sphere(tuple(self.x), tuple(self.__R__),self.__Rsig__[0], tuple(rho),tuple(eirho),
